...
class RoomService {
    ...
    async createRoom(nickName: string) {
        return this.axios.post<JoinRoomRespond>('/rooms', { hostName: nickName }).then(res => res.data);
    }

    async joinRoom(passCode: string, nickName: string) {
        return this.axios.post<JoinRoomRespond>(`/rooms/${passCode}/players`, {
            passCode,
            playerName: nickName
        })
            .then(res => res.data);
